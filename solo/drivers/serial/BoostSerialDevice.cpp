#include "solo/drivers/serial//BoostSerialDevice.hpp"
...
BoostSerialDevice::BoostSerialDevice(std::string port, uint32_t baudRate,
                                     bool isRequestEchoed)
  : port_(std::move(port))
    , baudRate_(baudRate)
...
Status BoostSerialDevice::open() {
  ...
  return Status(ErrorCode::CONTROL_ERROR, "ERROR_OPENING_DEVICE");
}
...
Status BoostSerialDevice::close() {
  ...
  return Status::OK(); // Port was not open
}

bool BoostSerialDevice::isValidRequest(const std::string& request,
...
Status BoostSerialDevice::innerWrite(const std::string& request) {
  ...
  if (isRequestEchoed_) {
    ...
    AINFO << "verifying device received request";
    ...
    if (!isValidRequest(request, echo)) {
      ...
      return Status(ErrorCode::CONTROL_ERROR, "COMMUNICATION_ERROR");
    }
  }
  return Status::OK();
}
...
Status BoostSerialDevice::write(const std::string& request) {
  ...
  if (!isOpened_) {
    ...
    return Status(ErrorCode::CONTROL_ERROR, "SENSOR_FAILURE");
  }
  return innerWrite(request);
}

std::tuple<Status, std::optional<std::string>> BoostSerialDevice::read(
  ...
  return {Status::OK(), std::make_optional(received)};
}
