...
public:
  ...
  BoostSerialDeviceManager& operator
  =(BoostSerialDeviceManager&& other) noexcept {
    if (this != &other) {
      std::lock_guard<std::mutex> lock(other.deviceMutex_);
  ...
  std::tuple<Status, std::optional<std::string>> readFromDevice(
      const std::string& port, const std::string& request);
...
private:
  ...
  std::unordered_map<std::string, std::unique_ptr<BoostSerialDevice>>
  deviceMap_;
