...
#include "solo/modules/drivers/serial/IOInterface.hpp"
...
public:
  ...
  BoostSerialDevice(std::string port, uint32_t baudRate,
                    bool isRequestEchoed = false);
  ...
  std::tuple<Status, std::optional<std::string>> read(
...
private:
  ...
  ///< Boost.Asio IO context for managing I/O services.
  boost::asio::serial_port serialPort_; ///< Boost.Asio serial port object.
  bool isRequestEchoed_;
  ///< Flag indicating whether the device echoes back the sent requests.
  boost::asio::streambuf response_;
  ///< Buffer for storing responses from the serial port.
};
} // namespace solo::drivers::serial