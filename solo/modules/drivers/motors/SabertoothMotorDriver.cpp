#include "platform/actuators/SabertoothMotorDriver.hpp"

#include "platform/io/Status.hpp"

namespace solo::drivers::motors {

SabertoothMotorDriver::SabertoothMotorDriver(const std::string& port, uint32_t baudRate)
    : port_(port)
    , baudRate_(baudRate)
{
...
io::Status SabertoothMotorDriver::driveMotor(double normalizedSpeed)
{
    ...
    if (std::abs(normalizedSpeed) < std::numeric_limits<double>::epsilon()) {
        ...
        speedCommand = 64 + static_cast<int>(normalizedSpeed * 63); // Negative speed adjusts properly
    }
...
io::Status SabertoothMotorDriver::stopMotor()
{
    return driveMotor(0.0); // Stop the motor
}


SabertoothMotorDriver::SabertoothMotorDriver(SabertoothMotorDriver&& other) noexcept
...
SabertoothMotorDriver& SabertoothMotorDriver::operator=(SabertoothMotorDriver&& other) noexcept
{
    if (this != &other) {
        port_ = std::move(other.port_);
        baudRate_ = other.baudRate_;
