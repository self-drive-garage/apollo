...
SteeringServoMotorController::SteeringServoMotorController(const PCA9685Config& config)
    : config_(config)
{
    begin();
...
void SteeringServoMotorController::reset()
{
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(10)); // Allow oscillator to stabilize
}
...
void SteeringServoMotorController::setPWM(uint8_t channel, uint16_t on, uint16_t off) const
{
...
void SteeringServoMotorController::setServoAngleRadians(double radians, const uint8_t channel) const
{
    radians = std::clamp(radians, -std::numbers::pi / 2, std::numbers::pi / 2); // Clamp angle to -π/2 to π/2 radians
    int pulse = config_.SERVOCENTER + static_cast<int>((radians / (std::numbers::pi / 2)) * ((config_.SERVOMAX - config_.SERVOMIN) / 2));
...
void SteeringServoMotorController::writeRegister(const uint8_t reg, const uint8_t value) const
{
